...
function assignFrequency(data) {
	...
	if(allReturned) {console.log("Frequencys done!"); sentenceComplete(wordsInSentence);}
}
...
function assignOnlyNoun(data) {
	...
	array.forEach(function(arrayitem, arrayindex) {
		...
		if(data[0]!=undefined&&data[0].word!=undefined) {
			...
			//console.log("array word: " + array[arrayindex].word + ", compared to data[0].word: " + data[0].word);
		...
		for (var u = 0; u < array.length; u++) {
			//console.log(array[u].returned);
			if (array[u].onlynounreturned == false) {allReturned = false; console.log(array[u].word);}
			
		}
						
	
	
	if(allReturned) {console.log("Onlynouns done!"); sentenceComplete(wordsInSentence);}
}
...
function sentenceComplete(array) {
	...
	if(allReturned) {findAnswer(array);}
