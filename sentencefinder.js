...
function findSentence(source) {
	for(var i = 0; i < source.length; i++) {
		var theSentence = source[i].abstract;
		console.log(theSentence);
		if(canUseSentence(theSentence)) {console.log("We can use it!");} else {console.log("Cant use it");}
	}
}

function canUseSentence(string) {
	var theLetters = string.toLowerCase().split(""),
		theWords = formatTheSentence(string),
	...
	for(var i = 0; i < theLetters.length; i++) {
		...
		if(theLetters[i] == "." && i != theLetters.length-1) {
			...
			if(title !="mr" && title != "ms" && title != "dr") {
				//console.log("2 sentences");
				return false;
			}
		}
	}
	//So far all flags have been avoided
	
	//Use the WordsAPI to ask for the word's frequency and part of speech
	
	for(var i = 0; i < theWords.length; i ++) {
		getData(theWords[i].word, createArray);
	}
...
function getData(source, callback) {
			...
			/*
			$.ajax({
			url: 'https://wordsapiv1.p.mashape.com/words/' + source, // The URL to the API. You can get this in the API page of the API you intend to consume
			type: 'GET', // The HTTP Method, can be GET POST PUT DELETE etc
			data: {}, // Additional parameters here
			dataType: 'json',
			success: function(data) {callback(data, wordsInSentence)},
